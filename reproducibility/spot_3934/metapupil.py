...
if __name__ == '__main__':
    ...
    decSI = torchmfbd.DeconvolutionMetapupils('spot_3934_meta.yaml')

    # Patchify and add the frames
    frames_patches = [None] * 2
    for i in range(2):        
        frames_patches[i], XY = patchify.patchify(warped[:, i, :, 0:512, 0:512], patch_size=32, stride_size=10, flatten_sequences=True, return_coordinates=True)
        noise = torchmfbd.compute_noise(frames_patches[i][0:1, 0:1, ...])
        decSI.add_frames(frames_patches[i], id_object=i, id_diversity=0, diversity=0.0, sigma=noise, XY=XY*0.038)
    ...
    frames_PD, XY = patchify.patchify(warped[:, 2, :, 0:512, 0:512], patch_size=32, stride_size=10, flatten_sequences=True, return_coordinates=True)
    noise = torchmfbd.compute_noise(frames_PD[0:1, 0:1, ...])
    decSI.add_frames(frames_PD, id_object=0, id_diversity=1, diversity=div, sigma=noise, XY=XY*0.038)
    ...
    for i in range(2):
        ax[0, i].imshow(warped[0, i, 0, 0:500, 0:500])
    ...
    hdul.writeto(f'spot_3934_meta.fits', overwrite=True)