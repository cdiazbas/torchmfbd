...
if __name__ == '__main__':
    ...
    lam = 8
    npix = config['npix']
    obs_file = f"obs/spot_20200727_083509_8542_obs5_npix512_original.h5"
        ...
        for mod in range(4):
            for cam in range(2):
                wb, nb = readsst(root, 
                                    ...
                                    mod=mod, 
                                    seq=[0, 1], 
                                    xrange=[200, 200+config['npix']], 
                                    yrange=[200, 200+config['npix']],
                                    destretch=False,
                                    instrument='CRISP')
                wb_all.append(wb)
                nb_all.append(nb)

        wb = np.concatenate(wb_all, axis=0)
        nb = np.concatenate(nb_all, axis=0)
    ...
    # MOMFBD
    tmp = f2[0][0].data[:, ::-1][200-20:200-20+config['npix'], 200-14:200-14+config['npix']][n:-n, n:-n]
    contrast = 100 * np.std(tmp) / np.mean(tmp)
    ...
    # MOMFBD
    tmp = f2[1][0].data[:, ::-1][200-20:200-20+config['npix'], 200-14:200-14+config['npix']][n:-n, n:-n]
    contrast = 100 * np.std(tmp) / np.mean(tmp)
