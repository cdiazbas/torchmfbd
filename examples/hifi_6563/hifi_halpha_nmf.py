...
if __name__ == '__main__':
    ...
    config = {
            'n_modes': 150,
            'basis_type': 'nmf',
            'basis_file': '../basis_nmf/basis_torch_6563_nmf_150_r0_5_30.npz',
            'npix_apod': 12,
            'gpu': 0,
            'npix': 256,
            'ngrid_modes': 16,
    ...
    f2 = [fits.open(f'aux/outbb.fz.fits'), fits.open(f'aux/outnb_pos0001.fz.fits')]
    ...
    # MOMFBD
    tmp = f2[0][0].data[:, :][200-20:200-20+config['npix'], 200-14:200-14+config['npix']][n:-n, n:-n]
    contrast = 100 * np.std(tmp) / np.mean(tmp)
    ...
    ax[0, 2].set_title(f'NMF-MOMFBD {contrast:.2f}%')
    ...
    # MOMFBD
    tmp = f2[1][0].data[:, :][200-20:200-20+config['npix'], 200-14:200-14+config['npix']][n:-n, n:-n]
    contrast = 100 * np.std(tmp) / np.mean(tmp)
    ...
    ax[1, 2].set_title(f'NMFs-MOMFBD {contrast:.2f}%')
    ...
    k_old, power_old = az_average.power_spectrum(f2[0][0].data[:, ::-1][300-20:300-20+config['npix'], 400-14:400-14+config['npix']][n:-n, n:-n])
